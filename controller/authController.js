...
const { JWT_SECRET_KEY, WEB_LINK, PORTAL_PASSWORD, PORTAL_EMAIL } = process.env
            ...
            if (user) {
                ...
                if (password?.length > 7) {
                    ...
                    if (savedUser.errors) {
                        ...
                        res.cookie("access_token", token, { Credential: "true" });
    ...
    try {
        ...
        if (email) {
            ...
            if (user) {
                ...
                const link = `${WEB_LINK}/api/auth/resetPassword/${user._id}/${token}`;
                // console.log(link, "==>> link")
                const transporter = nodemailer.createTransport({
                    service: 'gmail',
                    auth: {
                        user: PORTAL_EMAIL,
                        pass: PORTAL_PASSWORD,
                    },
                });

                const mailOptions = {
                    from: PORTAL_EMAIL,        //email jis se bhejni ho
                    to: PORTAL_EMAIL,         //jisko email bhejni ho
                    subject: 'Reset Password',
                    text: `Please click on the link to reset your password ${link}`,
                };
                console.log(mailOptions, " ==>> mail optins")
                transporter.sendMail(mailOptions, (error, info) => {
                    if (error) {
                        console.log(error);
                        return res
                            .status(INTERNALERROR)
                            .send(sendError({ status: false, message: error.message + " ===>> error aa rha ahi 287" }));
                    } else {
...
export const resetPasswordEmail = async (req, res) => {
    ...
    try {
        const { newPassword, confirmNewPassword, token }
            = req.body;
        if (newPassword && confirmNewPassword && token) {
            // console.log(token, " ====>> token 312")
            const { result } = verify(token, JWT_SECRET_KEY);
            // console.log(result, " ====>> result 314")
            const userId = result.slice(0, result.length - JWT_SECRET_KEY.length);
            const user = await Users.findById(userId);
            // return res.send(user)
            console.log(user, " ===>>> user in reset password email")
            if (user) {
                const salt = genSaltSync(10);
